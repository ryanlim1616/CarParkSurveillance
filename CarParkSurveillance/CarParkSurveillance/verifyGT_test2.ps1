...
$gt = Import-Csv C:\powershell\revisitGT.csv
...
if($debugON -eq $true)
{ 
    write-host "HEADER  i,state, time : " $i " , " $gt[$i].o_state  " , " $gt[$i].t_time
}
    ...
    for($j = $tempJ; $j -lt $db.Length; $j++)
    {
        ...
        if($gt[$i].o_state -eq $db[$j].obj_state)
        {
             if([datetime]$gt[$i].t_time + $time_window -ge [datetime]$db[$j].track_time -and    
                [datetime]$gt[$i].t_time - $time_window -le [datetime]$db[$j].track_time)
                {
                        ...
                        if($debugON -eq $true)
                        { 
                                write-host "                                        WRITE : " $($gt[$i].t_time) "," $($gt[$i].o_state) "," $($db[$j].track_time) "," $($db[$j].obj_state)
                        }
        ...
        if([datetime]$db[$j].track_time -gt [datetime]$gt[$i].t_time )
        {
            ...
            if($debugON -eq $true)
                        { 
                                write-host "                                        WRITE : " ","  ","  $($db[$j].track_time) ","  $($db[$j].obj_state)
                        }
...
$false_positive = $dbLength - $true_positive
...
"*** FALSE POSITIVE ***" | out-file C:\powershell\output.txt -Append
